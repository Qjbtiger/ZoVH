...
class OurArguments(TrainingArguments):
    # Ours arguments
    opt_name: str = "zovh" # zeroth-order type; only support "mezo", "hizoo", "zovh" for now
    num_histories: int = 15 # number of histories to use in ZoVH
    zovh_lambda_hess: float = 0.5
    hess_scheduler_step: int = 1500 # whether to use hessian smoothness scheduler
    zoar_scheduler_step: int = -1 # whether to use zo adaptive lr scheduler
...
class Framework:
    ...
    def load_model(self):
        ...
        with count_time("Loading model with FP%d" % (16 if self.args.load_float16 else 32)):
            ...
            if self.args.head_tuning:
                ...
                print("Using regular AutoModelForCausalLM model with auto device mapping!")
                model = AutoModelForCausalLM.from_pretrained(
                    self.args.model_name,
                    config=config,
                    device_map='auto',
    ...
    def train(self, train_samples, eval_samples):
        ...
        if self.args.only_train_option and not self.args.non_diff:
            ...
            print("Wrapping the forward function to only compute loss on the option part...")
        ...
        logger.info("[Junbin] for debugging, not resuming from checkpoint")
